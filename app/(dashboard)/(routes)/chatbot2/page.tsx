...
import useMessageStoreChatbot2 from "@/hooks/message-store-chatbot2";

const Conversation = () => {
  const router = useRouter();
  const messages = useMessageStoreChatbot2();
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      const response = await axios.post("/api/chatbot-two", {
        userMessage: values.prompt,
        threadId: localStorage.getItem("chatbot2"),
        fileId: localStorage.getItem("chatbot2FileId") || "",
      });
