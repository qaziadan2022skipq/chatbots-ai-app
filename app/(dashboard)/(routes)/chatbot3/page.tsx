...
const Conversation = () => {
  ...
  useEffect(() => {
    // create new thread
    localStorage.setItem("chatbot3FileId", "");
    createThread();
  }, []);

  const createThread = async () => {
    const response = await axios.post("/api/create-thread");
    localStorage.setItem("chatbot3", response.data);
  };
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      const response = await axios.post("/api/chatbot-three", {
        userMessage: values.prompt,
        threadId: localStorage.getItem("chatbot3"),
        fileId: localStorage.getItem("chatbot3FileId") || "",
      });
